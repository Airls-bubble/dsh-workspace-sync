...
export function createSyncService({ rowConfig, log, storePath }) {
  ...
  let lastReport = null;
  /** Live progress for status(): {runId, phase, totalOps, doneOps, totalBytes, doneBytes, current} */
  let progress = null;
  ...
  function startPairOffer() {
    ...
    // 过期后 TXT 不能继续挂着：对端列表里会显示一个永远点不动的「待配对」
    offerTimer = setTimeout(() => {
      if (pairOffer && Date.now() >= pairOffer.expiresAt) {
        pairOffer = null;
        if (advertise) advertise.setTxt({ pair: "" }).catch(() => {});
        log("pairing offer expired, beacon cleared");
      }
    }, OFFER_TTL_MS + 1500);
    if (offerTimer && offerTimer.unref) offerTimer.unref();
   ...
   *  mDNS multicast drops packets on WiFi, so a second browse may miss the
   *  peer even though the panel's earlier snapshot saw it. Fallback: try
   *  every online device directly — the claim route validates the code, a
   *  wrong code just 404s, so brute-forcing the beacon is unnecessary. */
  async function pairByShortCode(code) {
    const want = String(code).trim();
    const online = (await pairDiscover()).online || [];
    const hit = online.find((d) => d.pair === want);
    if (hit) return claimFromPeer(hit.url, code);
    let lastErr = null;
    for (const d of online) {
      try { return await claimFromPeer(d.url, code); } catch (e) { lastErr = e; }
    }
    if (lastErr) {
      const m = String((lastErr && lastErr.message) || lastErr);
      throw new Error(m.includes("短码无效") ? "在线设备都不认这枚短码（可能已过期或被作废），请让对方重新生成。" : "短码配对失败：" + m);
    }
    throw new Error("局域网里没有正在等待这枚短码的设备（确认对端已生成短码、两机在同一网络）");
  }
  ...
  async function runSync({ peerId, seed, confirmConflicts, background } = {}) {
    ...
    const runId = stamp();
    const t0 = Date.now();
    progress = { runId, phase: "scan", totalOps: 0, totalBytes: 0, doneOps: 0, doneBytes: 0, current: "" };
    try {
      const root = currentRoot();
      ...
      // ---- progress totals: bytes for put ops come from the source manifest ----
      const bytesOf = (op) => {
        if (op.op !== "put") return 0;
        const le = local.manifest.entries && local.manifest.entries[op.path];
        const re = remote.manifest.entries && remote.manifest.entries[op.path];
        return (le && le.size) || (re && re.size) || 0;
      };
      const allOps = (plan.localOps || []).concat(plan.remoteOps || []);
      Object.assign(progress, {
        phase: "apply",
        totalOps: allOps.length,
        totalBytes: allOps.reduce((a, op) => a + bytesOf(op), 0),
      });

      // ---- apply + commit, extracted so it can run in the background ----
      const execute = async () => {
        try {
          if (progress) progress.phase = "commit";
          const errors = [];
      ...
      for (const op of plan.localOps) {
        try {
          if (progress) progress.current = op.path;
          if (op.op === "put") {
          ...
          appliedLocal++;
          if (progress) { progress.doneOps++; progress.doneBytes += bytesOf(op); }
        } catch (e) {
