...
.wss-backdrop{pointer-events:auto;position:fixed;inset:0;display:flex;justify-content:center;align-items:center;background:rgba(0,0,0,.45);z-index:1000}
...
`;
		...
		const setOpen = (v) => { if (panelOpen === v) return; panelOpen = v; for (const l of listeners) l(); };
		...
		function Trigger(props) {
			const open = uSES(subscribe, getOpen, getOpen);
			return El("button", {
				className: "wss-btn" + (open ? " primary" : ""),
				style: props.wide ? null : { padding: "6px" },
				onClick: toggle,
				title: "工作区同步",
			}, props.wide ? (open ? "关闭面板" : "工作区同步") : "⇄");
		}
