...
`;
		...
		function Panel(props) {
			...
			const doSync = (confirmConflicts) => act(confirmConflicts ? "确认同步" : "同步", async () => {
				const r = await api("runSync", { confirmConflicts: !!confirmConflicts });
				setLastRun(r);
				...
				El("div", { className: "wss-body" },
					...
					El("div", { className: "wss-card" },
						...
						status ? [
							...
							El(Kv, { key: "r", k: "工作区", v: status.workspaceRoot }),
						] : El("div", null, "读取中…")),
					// 对端
					El("div", { className: "wss-card" },
						El("h3", null, "已配对对端"),
					...
					El("div", { className: "wss-card" },
						...
						El("p", { className: "wss-note" }, "冲突时先报计划不动手；确认后新者占原路径、旧者以 .conflict-<时间戳> 在两边共存；删除进回收站，绝不硬删。")),
					// 运行结果
