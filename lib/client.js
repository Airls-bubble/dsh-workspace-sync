...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const PHASE_TEXT = { scan: "扫描中", plan: "比对计划", apply: "传输文件", commit: "收尾" };
...
.wss-prog{margin-top:10px;display:flex;flex-direction:column;gap:6px}
.wss-prog-label{font-size:12px;color:var(--wss-text2)}
.wss-prog-bar{height:8px;border-radius:4px;background:var(--wss-hover);border:1px solid var(--wss-border);overflow:hidden}
.wss-prog-fill{height:100%;border-radius:4px;background:var(--wss-brand);transition:width .3s ease}
...
`;
		...
		function Panel(props) {
			...
			const lastRunRef = react.useRef ? react.useRef(null) : { current: null };
			const setRun = (r) => { lastRunRef.current = r; setRun(r); };
			f(() => { injectStyle(); refresh(); }, []);
			// 后台同步期间每 3 秒取一次状态：进度条由 status().progress 驱动，
			// 完成后把 status().lastReport 桥接成一次「已结束」的运行结果。
			f(() => {
				const timer = setInterval(async () => {
					try {
						...
						if (prev && prev.status === "started" && s.lastReport && s.lastReport.runId && s.lastReport.runId !== (prev.report && prev.report.runId)) {
							setRun({ ok: true, status: "synced", report: s.lastReport });
							setMsg({ ok: true, text: "后台同步已完成，结果在下方「运行结果」。" });
						}
					} catch {}
				}, 3000);
			...
			return El("div", { className: "wss-wrap" },
				El("div", { className: "wss-body" },
					...
					El("div", { className: "wss-card" },
						...
						status && status.progress ? El("div", { className: "wss-prog", key: "prog" },
							El("div", { className: "wss-prog-label" }, "同步中 · " + (PHASE_TEXT[status.progress.phase] || status.progress.phase) + " · " + status.progress.doneOps + "/" + status.progress.totalOps + " 个文件 · " + fmtBytes(status.progress.doneBytes) + " / " + fmtBytes(status.progress.totalBytes)),
							El("div", { className: "wss-prog-bar" }, El("div", { className: "wss-prog-fill", style: { width: (status.progress.totalOps ? Math.round((status.progress.doneOps / status.progress.totalOps) * 100) : 0) + "%" } })),
							status.progress.current ? El("p", { className: "wss-note" }, "正在处理：" + status.progress.current) : null) : null,
