...
.wss-backdrop{pointer-events:auto;position:fixed;inset:0;display:flex;justify-content:center;align-items:center;background:0 0;z-index:100}
...
.wss-pin{position:fixed;left:10px;bottom:10px;z-index:1001;pointer-events:none;font:11px ui-monospace,Menlo,monospace;background:#4d6bfe;color:#fff;border-radius:6px;padding:2px 8px;opacity:.85}
.wss-modal{pointer-events:auto;width:600px;max-width:94vw;height:min(760px,92vh);display:flex;flex-direction:column;position:relative;overflow:hidden;border-radius:14px;border:1px solid var(--wss-border,#3a3a45);background:var(--wss-bg,#1b1b22);color:inherit;box-shadow:0 8px 40px rgba(0,0,0,.25),0 2px 8px rgba(0,0,0,.15)}
...
`;
		...
		function ModalShell() {
			...
			if (!open) return null;
			return [
				El("div", { className: "wss-backdrop", key: "bd" },
					El("div", { className: "wss-modal" },
						El(Panel, { onClose: () => setOpen(false) }))),
				El("div", { className: "wss-pin", key: "pin" }, "wss open"),
			];
		}
		...
		exports.__test = { setOpen, getOpen, subscribe };
