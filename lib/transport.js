...
export function createSyncServer(opts) {
  ...
  const getPairOffer = typeof opts.getPairOffer === "function" ? opts.getPairOffer : () => null;
  // pairing short-code brute-force guard: per-IP failure counter → 30s lockout
  const claimFails = new Map();
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      // --- pairing short-code claim: no token yet (this is how you GET one) ---
      if (route === "POST /sync/pair-claim") {
        const ip = (req.socket && req.socket.remoteAddress) || "?";
        const lockedUntil = claimLocks.get(ip) || 0;
        if (Date.now() < lockedUntil) {
          res.writeHead(429, { "content-type": "application/json" });
          res.end(JSON.stringify({ error: "尝试过于频繁，请 30 秒后再试" }));
          return;
        }
        let body = {};
        try { body = JSON.parse(await readBody(req)) || {}; } catch {}
        const offer = getPairOffer();
        const given = String(body.code || "").trim();
        if (!offer || !/^\d{6}$/.test(given) || given !== offer.short) {
          const n = (claimFails.get(ip) || 0) + 1;
          claimFails.set(ip, n);
          if (n >= 5) {
            claimLocks.set(ip, Date.now() + 30000);
            claimFails.set(ip, 0);
            log("pair-claim locked out " + ip + " after 5 wrong codes");
          }
          res.writeHead(404, { "content-type": "application/json" });
          res.end(JSON.stringify({ error: "短码无效或已过期" }));
          return;
        }
        claimFails.delete(ip);
        log("pair claimed via short code by " + ip);
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({ ok: true, code: offer.long }));
        return;
      }
