...
import { scanManifest, readBaseline, writeBaseline } from "./manifest.js";
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (route === "POST /sync/manifest") {
        ...
        const { manifest, skipped } = await scanManifest(root, baseline);
        log("manifest served: " + Object.keys(manifest.entries).length + " entries");
        res.writeHead(200, { "content-type": "application/json" });
        res.end(JSON.stringify({ manifest, skipped }));
        return;
      }
      ...
      if (route === "POST /sync/baseline") {
        ...
        const baseline = await readBaseline(root, fsp);
        const { manifest } = await scanManifest(root, baseline);
